...
{/*
    ...
    float throttle = 0.0f;
    float reverse = 0.0f;
    Vector2 steerInput = Vector2.zero;
    public int bhp;
    float torque;
    public int brakeTorque;
    public int HandBrakeTorque;
    ...
    public Transform FLTrans;
    public Transform FRTrans;
    public Transform RLTrans;
    public Transform RRTrans;
    ...
    private void Awake()
    {
        ...
        drivingControls.Driving.HandBrake.started += ctx => handBrake = true;
        drivingControls.Driving.HandBrake.canceled += ctx => handBrake = false;

        drivingControls.Driving.Throttle.performed += ctx => throttle = ctx.ReadValue<float>();
        drivingControls.Driving.Throttle.canceled += ctx => throttle = 0.0f;

        drivingControls.Driving.Reverse.performed += ctx => reverse = -ctx.ReadValue<float>();
        drivingControls.Driving.Reverse.canceled += ctx => reverse = 0.0f;
...
/*
    ...
    void Accelerate()
    {
        float vertical = throttle + reverse;
        float appliedTorque = vertical * torque;
        Debug.Log(currentSpeed + "   ||||||||||||||||" + count++);
        if (currentSpeed < maxSpeed && engineRPM <= gearUpRPM)
        {

            RL.motorTorque = torque * vertical;
            RR.motorTorque = torque * vertical;
            RL.brakeTorque = 0;
            RR.brakeTorque = 0;
        }
        else
        ...
        if (engineRPM > 0 && vertical < 0 && engineRPM <= gearUpRPM)
        {
    ...
    void Steer()
    {
        ...
        FL.steerAngle = maxSteerAngle * steerAngle;
        FR.steerAngle = maxSteerAngle * steerAngle;
    }
    ...
    {
        UpdateWheelPose(FL, FLTrans);
        UpdateWheelPose(FR, FRTrans);
        UpdateWheelPose(RL, RLTrans);
        UpdateWheelPose(RR, RRTrans);
    }

    void UpdateWheelPose(WheelCollider wheel, Transform trans)
    {
    ...
    {
        steerInput = drivingControls.Driving.Steering.ReadValue<Vector2>();

        steerAngle = steerInput.x;
